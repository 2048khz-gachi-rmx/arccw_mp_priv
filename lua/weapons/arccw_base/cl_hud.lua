...
function SWEP:GetHUDData()
	data.clip 			= math.Round(vclip or self:Clip1())
	data.ammo 			= math.Round(vreserve or self:Ammo1())
	data.bars 			= self:GetFiremodeBars()
	data.mode 			= self:GetFiremodeName()
	data.ammotype 		= self.Primary.Ammo
	data.heat_enabled   = self:HeatEnabled()
	data.heat_name		= "HEAT"
	data.heat_level		= self:GetHeat()
	data.heat_maxlevel	= self:GetMaxHeat()
	data.heat_locked	= self:GetHeatLocked()
...
function SWEP:DrawHUD()
	...
	local apan_bg = lazy.Get("fuckingacw") or lazy.Set("fuckingacw", {})
	apan_bg.w = ScreenScaleMulti(128)
	apan_bg.h = ScreenScaleMulti(48)
	...
	if ArcCW:ShouldDrawHUDElement("CHudAmmo") then
		...
		local curInfo = lazy.Get("fuckingacw_curInfo") or lazy.Set("fuckingacw_curInfo", {})
			curInfo.ammo = data.ammo
			curInfo.clip = data.clip
			curInfo.plus = data.plus
			curInfo.ammotype = data.ammotype
			curInfo.firemode = data.mode
			curInfo.heat = data.heat_level
			curInfo[1] = self:GetInUBGL()
			curInfo[2] = self:InBipod()
			curInfo[3] = self:CanBipod()
		...
		if alpha > 0 then
			...
			if GetConVar("arccw_hud_3dfun_ammotype"):GetBool() then
				local wammotype = lazy.Get("acw_blammo") or lazy.Set("acw_blammo", {})
					wammotype.x = apan_bg.x + apan_bg.w - airgap
					wammotype.y = apan_bg.y - ScreenScaleMulti(8)
					wammotype.text = language.GetPhrase(data.ammotype .. "_ammo")
					wammotype.font = "ArcCW_8"
					wammotype.col = col2
					wammotype.align = 1
					wammotype.shadow = true
					wammotype.alpha = alpha
