	...
	for i, k in pairs(models) do
		...
		if bonename then
			...
			if custompos and (!self.MirrorVMWM or (self.MirrorVMWM and k.Model:GetModel() == self.ViewModel) ) then
				...
				SHARED_VEC:SetUnpacked(
					bonemat:GetField(1, 4),
					bonemat:GetField(2, 4),
					bonemat:GetField(3, 4)
				)
			...
			if k.Slot then
				...
				for _, e in pairs(self:GetActiveElements()) do
					...
					if ((ele.AttPosMods or EMPTY_TBL)[k.Slot] or EMPTY_TBL).vpos then
					...
					if !self.MirrorVMWM then
						if ((ele.AttPosMods or EMPTY_TBL)[k.Slot] or EMPTY_TBL).wpos then
							wmelemod = ele.AttPosMods[k.Slot].wpos
						end
					end

					if ((ele.AttPosMods or EMPTY_TBL)[k.Slot] or EMPTY_TBL).slide then
					...
					if ((ele.AttPosMods or EMPTY_TBL)[k.Slot] or EMPTY_TBL).SlideAmount then
						slidemod = ele.AttPosMods[k.Slot].SlideAmount
					end
				end

				if wm and !self.MirrorVMWM then
					offset = wmelemod or (attslot.Offset or EMPTY_TBL).wpos or Vector(0, 0, 0)
					...
					offset = vmelemod or (attslot.Offset or EMPTY_TBL).vpos or Vector(0, 0, 0)
...
function SWEP:GetFromReference(boneid)
	...
	elseif not bpos then
		bpos = ArcCW.ReferenceModel:GetPos() -- better than erroring i guess
		bang = ArcCW.ReferenceModel:GetAngles()
	end
