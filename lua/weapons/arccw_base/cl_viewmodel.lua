...
function SWEP:GetViewModelPosition(pos, ang)
    ...
    local intensity = math.Clamp(0.6 / self:GetSightTime(), 1, 3)
    ...
    if sprinted then
        ...
        local fracSinceSprint = math.min( 1, (UCT - self.LastEnterSprintTimeUnpred) / recovery )
        local fracSinceUnsprint = math.min( 1, (UCT - self.LastExitSprintTimeUnpred) / recovery )

        if t.LastExitSprintTimeUnpred >= t.LastEnterSprintTimeUnpred then
            local frSprint = easeOut( fracSinceSprint, intensity )
            sprintFrac = frSprint - easeOut( fracSinceUnsprint, intensity )
        else
            local frUnsprint = easeOut(fracSinceUnsprint, intensity)
            sprintFrac = 1 - (frUnsprint - easeOut( fracSinceSprint, intensity ) * frUnsprint)
        end
