...
function SWEP:RecalcAllBuffs()
	...
	t.TickCache_Mults = {}
	t.TickCache_Hooks = {}
	t.TickCache_IsShotgun = nil
	t.TickCache_IsShotgun = self:GetIsShotgun()

	t.TickCache_Tick_Overrides = {}
	t.TickCache_Tick_Adds = {}
	t.TickCache_Tick_Mults = {}

	t.AttCache_Hooks = {}
	t.ActiveElementCache = nil
end
...
function SWEP:GetBuff_Hook(buff, data)
	...
	local hks = t.AttCache_Hooks[buff]
	if hks then

		for i, k in ipairs(hks) do
...
function SWEP:GetBuff_Override(buff, default)
	...
	local t = self:GetTable()
	local ov = t.TickCache_Overrides[buff]
...
function SWEP:GetActiveElements(recache)
	...
	if t.ActiveElementCache and !recache then
		return t.ActiveElementCache
	end

	if ArcCW.Overflow and t.ActiveElementCache then
		return t.ActiveElementCache
	end

	--[[print("RECACHE", recache)
	print(debug.traceback())]]

	local eles = {}

	for _, i in pairs(t.Attachments) do
		if !i.Installed then
	...
	for f, i in pairs(eles) do
		local e = t.AttachmentElements[i]

		if !e then continue end

		if !t:CheckFlags(e.ExcludeFlags, e.RequireFlags) then continue end
